...
for ($ch = 1; $ch <= 3; $ch++){ #chapter counter
	...
	for ($chpn = 1; $chpn <= 8; $chpn++){ # the page within the chapter 
		...
		if ($chpn == 1){
			...
			# make a regular page
			print "$pn: regular page\t" . length($chapters{$ch}) ."\n" if $verbose == 1;
...
sub makeRegularPage {
	...
	# temporary like, for testing

	system("convert -size 1000x1600 xc:gray img/tmp/layout.png");
	addPageNumber();
...
#	$frame = getFrame();
	# also move a copy of this image to the chapter cover image folder
 	...
 	my $layout = `convert -size 1000x1600 xc:blue img/tmp/layout.png`;

 	return;

	$frame = "img/tmp/covers/ch-2.png";
